...
def known(db: 'Database', t1: Triangle, t2: Triangle) -> bool:
    ...
    return t1 != t2 and t1 in db.inverse_simtri and t2 in db.simtri[
        db.inverse_simtri[t1]]
...
def init(db: 'Database'):
    ...
    db.simtri = OrderedSet(
    )  # OrderedSet[Triangle, OrderedSet[Triangle]] (rep -> eqclass)
    db.inverse_simtri = {}  # Dict[Triangle, Triangle] (elem -> rep)
    db.segments_simtris = {}  # Dict[Segment, OrderedSet[Triangle]]
    db.points_simtris = {}  # Dict[Point, OrderedSet[Segment]]
...
def predicate_to_fact(predicate: 'Predicate') -> 'Fact':
    ...
    if len(predicate.objects) == 2:  # two Triangles
        return Fact("simtri", predicate.objects)
    A, B, C, D, E, F = predicate.objects  # six points
    return Fact("simtri", [Triangle(A, B, C), Triangle(D, E, F)])
...
def add_fact(db: 'Database', fact: 'Fact') -> List['Fact']:
    """Add Fact(simtri, [t1, t2]). Be aware that the order of triangle
        vertices matters."""
    Fact = tonggeometry.inference_engine.predicate.Fact
    facts_from_add = []
    ...
    if t1 not in db.inverse_simtri and t2 not in db.inverse_simtri:  # pylint: disable=too-many-nested-blocks
        if t1 > t2:
            t1, t2 = t2, t1
        db.simtri[t1] = OrderedSet(zip([t1, t2], [t1, t2]))
        db.inverse_simtri[t1] = t1
        db.inverse_simtri[t2] = t1
        for t in [t1, t2]:
            for name in combinations(t.name, 2):
                s = Segment(*name)
                if s in db.segments_simtris:
                    db.segments_simtris[s][t] = None
                else:
                    db.segments_simtris[s] = OrderedSet.fromkeys([t])
                    for p in [s.p1, s.p2]:
                        if p in db.points_simtris:
                            db.points_simtris[p][s] = None
                        else:
                            db.points_simtris[p] = OrderedSet.fromkeys([s])
    elif not (t1 in db.inverse_simtri and t2 in db.inverse_simtri):
        if t1 not in db.inverse_simtri:
            t1, t2 = t2, t1  # t1 existing, t2 new
        key = db.inverse_simtri[t1]
        t0 = db.simtri[key][t1]
        if t1.name != t0.name:
            indices = [t1.name.index(p) for p in t0.name]
            t2 = Triangle(*[t2.name[idx] for idx in indices])
            t1 = t0
        for t in db.simtri[key].values():
            if t == t1:
                continue
            f = Fact("simtri", [t, t2], "add_fact")
            f.add_parent(Fact("simtri", [t, t1]))
            f.add_parent(Fact("simtri", [t1, t2]))
            facts_from_add.append(f)
        db.simtri[key][t2] = t2
        db.inverse_simtri[t2] = key
        for name in combinations(t2.name, 2):
            s = Segment(*name)
            if s in db.segments_simtris:
                db.segments_simtris[s][t2] = None
            else:
                db.segments_simtris[s] = OrderedSet.fromkeys([t2])
                for p in [s.p1, s.p2]:
                    if p in db.points_simtris:
                        db.points_simtris[p][s] = None
                    else:
        ...
        t1_key = db.inverse_simtri[t1]
        t2_key = db.inverse_simtri[t2]
        if t1_key > t2_key:
            t1_key, t2_key = t2_key, t1_key
            t1, t2 = t2, t1
        t0 = db.simtri[t1_key][t1]
        if t1.name != t0.name:
            indices = [t1.name.index(p) for p in t0.name]
            t2 = Triangle(*[t2.name[idx] for idx in indices])
            t1 = t0
        to_merge = OrderedSet()
        eqclass = db.simtri.pop(t2_key)
        t0 = eqclass[t2]
        if t2.name != t0.name:
            ...
            for t in eqclass:
                ...
                for name in combinations(t.name, 2):
                    s = Segment(*name)
                    db.segments_simtris[s].pop(t)
                    db.segments_simtris[s][new_t] = None
                to_merge[new_t] = new_t
        else:
            to_merge.update(eqclass)
        for t in db.simtri[t1_key].values():
            for tt in to_merge.values():
                if t == t1 and tt == t2:
                    continue
                f = Fact("simtri", [t, tt], "add_fact")
                if t != t1:
                    f.add_parent(Fact("simtri", [t, t1]))
                f.add_parent(Fact("simtri", [t1, t2]))
                if tt != t2:
                    f.add_parent(Fact("simtri", [t2, tt]))
                facts_from_add.append(f)
        db.simtri[t1_key].update(to_merge)
        for k in to_merge:
            db.inverse_simtri[k] = t1_key
    return facts_from_add
